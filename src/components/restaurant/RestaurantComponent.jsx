                ...
                <Grid container spacing={24}>
                    {
                        restaurants.map(res => {
                            ...
                            if(res.restype_id !== null) {
                                ...
                                let i
                                for(i in restype_id) {
                                    const getType = restaurantTypes.filter(type => type.restype_id === restype_id[i])
                                    type = [...type, getType[0]]
                                }
                            }
                            console.log(type)
                            return <Grid component={Link} to={`/restaurant/${res.res_id}`} item xs={3} className="styledlink">
                                <div className="image-wrapper">
                                    <img src={`http://localhost:3000/restaurants/${res.res_logo}`} alt="" />
                                </div>
                                    ...
                                    <span className="headline">
                                        ...
                                        <div className="ratings-component">
                                            <span className="stars">
                                                <img alt="" src="https://assets.foodora.com/819a336/img/icons/ic-star-sm.svg?819a336" />
                                            </span>
                                            <span className="rating"><strong>4.6</strong>/5</span>
                                        </div>
                                    </span>
                                </div>
                                {
                                    type.map((t, idx) => {
                                    return <div className="restaurant-type-text">
                                        {`${t.restype_name}${(type.length > 0 && idx+1 < type.length ? ", " : " ")}`}
                                    </div>
                                    }) 
                                }
                            </Grid>
